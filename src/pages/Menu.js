...
const Menu = () => {
  ...
  const [formData, setFormData] = useState(EMPTY_MENU_FORM);
  ...
  return (
    <div className="menu-page">
      ...
      <Modal show={showAddForm || showEditForm} onHide={handleCloseMenuForm} size="lg" centered>
        <Modal.Header closeButton>
        ...
        <Modal.Body>
          <Form id="menu-form" onSubmit={handleSubmit}>
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  {formData.hasVariants && formData.variants.length > 0 && (
                    <Form.Text className="text-muted">
                      Price will be determined by the selected size/variant.
                    </Form.Text>
                  )}
                </Form.Group>
              </Col>
            ...
            {formData.hasVariants && (
              <div className="bg-light p-3 rounded mb-3 border border-primary-soft">
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <h6 className="mb-0 fw-bold">Sizes / Variants</h6>
                  <Button variant="outline-primary" size="sm" onClick={addVariant}>
                     + Add Size
                  </Button>
                </div>
                {formData.variants.length === 0 ? (
                  <div className="text-center py-2 text-muted small">No sizes added. Click + to add.</div>
                ) : (
                  formData.variants.map((v, idx) => (
                    <Row key={idx} className="g-2 mb-2 align-items-center">
                      ...
                      <Col xs={1}>
                        <Button variant="link" className="text-danger p-0" onClick={() => removeVariant(idx)}>
                          ✕
                        </Button>
                      </Col>
                    </Row>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={handleCloseMenuForm}>
            Cancel
          </Button>
      ...
      {filteredItems.length === 0 ? (
        ...
        <Row xs={1} md={2} lg={3} xl={4} className="g-4">
          {filteredItems.map(item => (
            <Col key={item._id}>
              <Card className={`h-100 glass-card border-0 menu-card-bs ${!item.isAvailable ? 'opacity-75' : ''}`}>
