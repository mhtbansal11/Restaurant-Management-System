...
const Orders = () => {
  ...
  const getOrderTableLabel = (order) => {
    if (Array.isArray(order?.tableLabels) && order.tableLabels.length > 0) {
      return order.tableLabels.join(' + ');
    }

    return order?.tableLabel || order?.tableId?.label || order?.tableId?.tableId || order?.tableId || 'N/A';
  };
    ...
    try {
      ...
      if (orderSearch.trim() !== '') {
        ...
        filtered = filtered.filter(order => {
          ...
          const matchesTable = getOrderTableLabel(order).toString().toLowerCase().includes(searchLower);
    ...
    <div className="orders-page-premium">
      ...
      <div className="orders-hero page-header-glass d-flex flex-wrap justify-content-between align-items-center mb-3 gap-3 p-3 rounded-4">
        <div className="flex-grow-1">
          ...
          <div className="d-flex flex-wrap gap-2 align-items-center">
            ...
            <span className="badge-pill-premium badge-pending">
              {pendingOrdersCount} Pending
            </span>
            <span className="badge-pill-premium badge-preparing">
      ...
      <Row xs={1} md={2} lg={3} xl={4} className="g-4">
        {orders.length === 0 ? (
          ...
          orders.map(order => (
            <Col key={order._id}>
              <Card className={`order-card-premium h-100 border-0 glass-card status-${order.status}`}>
                    ...
                    <div className="d-flex justify-content-between align-items-start mb-2">
                      <div>
                        ...
                        <small className="text-muted d-block mt-1">
                          #{order?._id?.slice(-6) || 'N/A'} • {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </small>
                      </div>
                      <Badge 
                    ...
                    <div className="d-flex flex-column gap-2">
                      <div className="d-flex gap-2 order-action-row">
                        {order.status !== 'cancelled' && (
                          <Button 
                            as={Link}
                            to={`/pos?tableId=${order.tableIds?.[0] || order.tableId?._id || order.tableId || ''}&orderId=${order._id}&type=${order.orderType || 'dine-in'}&tableLabel=${encodeURIComponent(getOrderTableLabel(order))}`}
                            variant="light"
