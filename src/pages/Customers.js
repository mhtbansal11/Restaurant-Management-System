    ...
    return (
      <Container className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
        <div className="text-center">
          ...
          <p className="mt-3 text-muted fw-bold">Synchronizing CRM data...</p>
        </div>
      </Container>
    );
  }
  ...
  return (
    <div className="customers-page-premium">
      ...
      <Row xs={1} md={2} xl={4} className="g-4 mb-4">
        <Col>
          <Card className="h-100 glass-card border-0 stat-card-bs">
      ...
      <Row xs={1} md={2} xl={3} className="g-4">
        {filteredCustomers.map(customer => (
          <Col key={customer._id}>
            <Card 
              className="h-100 glass-card border-0 customer-card-bs"
              onClick={() => handleViewDetails(customer)}
            >
              <Card.Header className="bg-transparent border-0 pt-3 pb-0 d-flex justify-content-between align-items-start">
                <div>
              ...
              <Card.Body className="d-flex flex-column">
                <div className="d-flex gap-2 mb-3 bg-light p-3 rounded-4 border">
                  <div className="text-center flex-grow-1">
                    <small className="text-muted d-block text-uppercase fw-bold" style={{fontSize: '0.65rem'}}>Orders</small>
                    <span className="fw-bold h5 mb-0">{customer.orderCount || 0}</span>
                  </div>
                  <div className="vr opacity-10"></div>
                  <div className="text-center flex-grow-1">
                    <small className="text-muted d-block text-uppercase fw-bold" style={{fontSize: '0.65rem'}}>Lifetime Value</small>
                    <span className="fw-bold h5 mb-0 text-indigo-600">₹{(customer.clv || 0).toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="d-flex justify-content-between align-items-center mb-3 px-1">
                  <div className="small">
                    ...
                    <span className="fw-bold">₹{Math.round(customer.averageOrderValue || 0).toLocaleString()}</span>
                  </div>
                  {customer.clv > 5000 && <Badge bg="indigo" style={{background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)'}} className="rounded-pill px-3 py-2 shadow-sm">💎 VIP Guest</Badge>}
                </div>
                    ...
                    <tbody>
                      {selectedCustomer.orderHistory.map(order => (
                        <tr key={order._id}>
                          ...
                          <td className="fw-medium">#{order?._id?.slice(-6) || 'N/A'}</td>
                ...
                >
                  ...
                  {dueOrderOptions.map(order => (
                      <option key={order._id} value={order._id}>
                        #{order?._id?.slice(-6) || 'N/A'} • Due ₹{(order.dueAmount || 0).toLocaleString()}
                      </option>
                  ))}
                </Form.Select>
              </Form.Group>
