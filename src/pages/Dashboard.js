...
console.log("topMenuItems",topMenuItems)
  ...
  const fetchStats = async () => {
    try {
      ...
      const totalTables = seatingLayout.floors
        ? seatingLayout.floors.reduce(
            (sum, floor) => sum + (floor.tables?.length || 0),
            0
          )
        : seatingLayout.tables?.length || 0;

      const completedOrdersTodayRes = await axios.get(
        `${config.ENDPOINTS.ORDERS}?status=completed`
      );
      ...
      const averageOrderValue =
        completedOrdersCount > 0 ? todayRevenue / completedOrdersCount : 0;
      ...
      allOrdersRes.data.forEach((order) => {
        order.items?.forEach((item) => {
          menuItemCounts[item.menuItem] =
            (menuItemCounts[item.menuItem] || 0) + item.quantity;
        });
      });
  ...
  const getTableStatus = (tableId) => {
    return (
      tableStatuses.find((t) => t.tableId === tableId)?.status || "available"
    );
  };
    ...
    try {
      if (action === "order") {
        if (selectedTable.status === "occupied" && selectedTable.currentOrder) {
          ...
          navigate(
            `/pos?tableId=${selectedTable.id}&orderId=${selectedTable.currentOrder}&type=dine-in&tableLabel=${selectedTable.label}`
          );
        } else {
          // New order - only tableId
          navigate(
            `/pos?tableId=${selectedTable.id}&type=dine-in&tableLabel=${selectedTable.label}`
          );
        }
      } else if (action === "book") {
        ...
        if (selectedTable.status === "occupied" && selectedTable.currentOrder) {
          toast.error(
            `Table ${selectedTable.label} has an active order. Complete or cancel it first.`
          );
  ...
  return (
    <div className="dashboard">
      {/* Header */}
      <div className="dashboard-header mb-4 p-3 rounded-4 bg-primary shadow-sm border">
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="flex-grow-1">
            <h2 className="mb-1 fw-bold text-primary">Restaurant Dashboard</h2>
      ...
      <Row className="g-4 mb-2">
        {ROLES.MANAGEMENT.includes(user?.role) && (
          <Col lg={3} md={6} sm={6} className="mb-3">
            <Card className="h-100 overflow-hidden bg-primary border-0">
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-muted small">
                      {stats.completedOrders || 0} orders completed
                    </div>
                  </div>
                </div>
              </Card.Body>
            ...
            <Card.Body className="p-3">
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  ...
                  <h4 className="mb-0 fw-bold">
                    {
                      tableStatuses.filter((t) => t.status === "occupied")
                        .length
                    }
                  </h4>
                  <div className="text-muted small">
                    out of {stats.totalTables} tables
                  </div>
                </div>
              </div>
            </Card.Body>
        ...
        {(ROLES.MANAGEMENT.includes(user?.role) ||
          ROLES.KITCHEN.includes(user?.role)) && (
          <Col lg={3} md={6} sm={6} className="mb-3">
      ...
      <Row>
        ...
        {(ROLES.POS.includes(user?.role) ||
          ROLES.FRONT_DESK.includes(user?.role)) && (
          <Col lg={12} className="mb-4">
            <Card className="floor-management-card bg-primary border-0">
              <Card.Header className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                  <h5 className="mb-1">🏢 Floor Management</h5>
                  <small className="text-muted">
                    Click tables to manage orders and reservations
                  </small>
                </div>
                <div className="d-flex gap-2 align-items-center">
                  <div className="d-flex gap-2">
                    <Badge
                      bg="success-subtle"
                      className="text-success border border-success-subtle px-2 py-1"
                    >
                      Available
                    </Badge>
                    <Badge
                      bg="danger-subtle"
                      className="text-danger border border-danger-subtle px-2 py-1"
                    >
                      Occupied
                    </Badge>
                    <Badge
                      bg="warning-subtle"
                      className="text-warning-emphasis border border-warning-subtle px-2 py-1"
                    >
                      Reserved
                    </Badge>
                    <Badge
                      bg="info-subtle"
                      className="text-info-emphasis border border-info-subtle px-2 py-1"
                    >
                      Cleaning
                    </Badge>
                  </div>
                ...
                {layout ? (
                  <div className="">
                    ...
                    {layout?.floors?.map((floor) => {
                      ...
                      return (
                        <div key={floor.id} className="mb-4">
                          <div className="d-flex align-items-center justify-content-between mb-3 p-3 bg-tertiary rounded-3 flex-wrap gap-2">
                            <div className="d-flex align-items-center">
                              <h6 className="mb-0 fw-bold text-primary">
                                🏢 {floor.name}
                              </h6>
                              <Badge bg="tertiary" text="primary" className="ms-2 border border-primary border-opacity-25">
                          ...
                          {(floorViewMode[floor.id] || "list") === "editor" ? (
                            ...
                            <div className="row g-3">
                              {floorTables.map((table) => {
                                ...
                                const paidAmount =
                                  tableStatus?.currentOrder?.paidAmount || 0;
                                const totalAmount =
                                  tableStatus?.currentOrder?.totalAmount ||
                                  tableStatus?.currentOrder?.subtotal ||
                                  0;
                                    ...
                                    >
                                      <Card.Body className="p-2 d-flex flex-column position-relative">
                                        <div className="d-flex justify-content-between align-items-start mb-2 flex-wrap gap-1">
                                          ...
                                          <div className="d-flex gap-1 align-items-center">
                                            ...
                                            {status === "available" && (
                                              <div
                                                role="button"
                                                className="btn btn-sm btn-tertiary border rounded-circle d-flex align-items-center justify-content-center p-0 text-primary"
                                                style={{
                                                  width: "24px",
                                                  height: "24px",
                                                  flexShrink: 0,
                                        ...
                                        <div
                                          role="button"
                                          onClick={() =>
                                            onTableTileClick(floor.id, table)
                                          }
                                            ...
                                            tableStatus?.currentOrder && (
                                              <div className="mt-2 text-center w-100">
                                                ...
                                                >
                                                  ...
                                                  {tableStatus.currentOrder
                                                    .orderNumber ||
                                                    tableStatus.currentOrder._id?.slice(
                                                      -4
                                                    )}
                                                </Badge>
                    ...
                    {offTableOrders.length > 0 && (
                      <div className="mt-4">
                        <div className="d-flex align-items-center mb-3 p-3 bg-tertiary rounded-3">
                          <h6 className="mb-0 fw-bold text-primary">
                            🥡📦 Takeaway & Packing Orders
                          </h6>
                          <Badge bg="secondary" className="ms-2">
                            {offTableOrders.length} order
                            {offTableOrders.length !== 1 ? "s" : ""}
                          </Badge>
                        </div>
                        <div className="row g-3">
                          {offTableOrders.map((order) => {
                            ...
                            return (
                              ...
                              >
                                <Card
                                  className="h-100 takeaway-order-card hover-lift shadow-sm bg-primary border-0"
                                  onClick={() =>
                                    navigate(
                                      `/pos?orderId=${order._id}&type=${order.orderType}`
                                    )
                                  }
                                >
                                  <Card.Body className="p-3 d-flex flex-column">
                                    ...
                                    <div className="order-type-header mb-3">
                                      <div className="d-flex align-items-center justify-content-between mb-2">
                                        <div className="d-flex align-items-center gap-2">
                                          <span className="order-type-emoji">
                                            {orderTypeIcon}
                                          </span>
                                    ...
                                    {(order.customerName ||
                                      ...
                                      <div className="customer-info mb-2">
                                        <div className="d-flex align-items-center gap-2">
                                          <div
                                            className="customer-avatar bg-tertiary rounded-circle text-primary fw-bold d-flex align-items-center justify-content-center"
                                            style={{
                                              width: "20px",
                                              height: "20px",
                                              fontSize: "0.6rem",
                ...
                {operationalInsights ? (
                  <>
                    <div className="d-flex gap-3">
                      <div className="intelligence-item mb-3 w-50">
                        <small className="text-muted d-block mb-1">
                          Peak Hour
                        </small>
                        <strong>
                    ...
                    <div className="intelligence-item">
                      ...
                      <div className="small">
                        {operationalInsights.summary || "Analyzing patterns..."}
                      </div>
                    </div>
                  </>
                ) : (
                ...
                {topMenuItems.length > 0 ? (
                  <div className="menu-items-list">
                    {topMenuItems.map((item) => (
                      <div
                        key={item._id}
                        className="menu-item d-flex justify-content-between align-items-center py-2 border-bottom"
                      >
                        <div>
                ...
                <Row>
                  ...
                  <Col lg={3} md={6} sm={6} className="mb-3">
                    <div className="financial-item">
                      ...
                      <div className="financial-value">
                        ₹{stats.onlineReceived.toLocaleString()}
                      </div>
                    </div>
                  </Col>
                  <Col lg={3} md={6} sm={6} className="mb-3">
      ...
      {/* Quick Actions */}
      <div
        className={`floating-quick-action ${isQuickActionOpen ? "active" : ""}`}
      >
        ...
        <Modal.Body>
          <div className="d-flex flex-column gap-3">
            <div className="text-center mb-3">
              ...
              <Badge
                bg={
                  settingsTable?.status === "available"
                    ? "success"
                    : settingsTable?.status === "occupied"
                    ? "danger"
                    : settingsTable?.status === "reserved"
                    ? "warning"
                    : "secondary"
                }
            ...
            {settingsTable?.status === "occupied" ? (
              <div className="alert alert-warning mb-0">
                <small>
                  ⚠️ This table is currently occupied. You cannot change its
                  status or delete it until the order is completed or cleared.
                </small>
              </div>
            ) : (
              <>
                <div className="d-grid gap-2">
                  <Button
                    variant={
                      settingsTable?.status === "maintenance"
                        ? "secondary"
                        : "outline-secondary"
                    }
                    onClick={() => {
                  ...
                  <Button
                    variant={
                      settingsTable?.status === "unavailable"
                        ? "secondary"
                        : "outline-secondary"
                    }
                    onClick={() => {
                ...
                <div className="d-grid">
                  <Button
                    ...
                    onClick={() => {
                      handleAskDeleteTable(
                        settingsTable.floorId,
                        settingsTable
                      );
                      setSettingsTable(null);
                    }}
