  ...
  const fetchStats = useCallback(async () => {
    try {
      const [statsRes, alertsRes] = await Promise.all([
        axios.get(`${config.ENDPOINTS.EXPENSES}/stats/summary?period=${filters.period}`),
        axios.get(`${config.ENDPOINTS.EXPENSES}/stats/alerts`)
      ]);
      setStats(statsRes.data);
      setAlerts(alertsRes.data || []);
    } catch (error) {
      console.error('Error fetching stats or alerts:', error);
    }
  }, [filters.period]);
    ...
    return (
      <Container className="py-4 d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
        <div className="text-center">
          ...
          <p className="mt-3 text-muted fw-bold">Synchronizing financial records...</p>
        </div>
      </Container>
    );
  }
